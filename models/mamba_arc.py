...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.conv = nn.Sequential(
            ...
            nn.GELU(),
            nn.Conv2d(hidden_dim // 3, hidden_dim, 3, 1, 1),
        )
        # self.conv_blk = CAB(hidden_dim)
        self.conv_blk = Block(hidden_dim, 1, 8)
    ...
    def forward(self, input, x_size):
        ...
        x = self.ln_1(input)
        x = nn.ZeroPad2d(padding=1)(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)

        x = input * self.skip_scale + self.drop_path(self.self_attention(x)[:, 1:-1, 1:-1])
        x_skip = x
        x = self.ln_2(x).permute(0, 3, 1, 2).contiguous()
        x = self.conv(x)
        B, c, h, w = x.shape
        x = x.view(B, c, h * w).permute(0, 2, 1)
        x = x_skip * self.skip_scale2 + self.conv_blk(x, h, w).view(B, h, w, c).contiguous()
    ...
    def __init__(self,
        ...
        for i in range(depth):
            self.blocks.append(VSSBlock(
                ...
                attn_drop_rate=0,
                d_state=d_state,
                expand=self.mlp_ratio, input_resolution=input_resolution, is_light_sr=is_light_sr))
...
if __name__ == "__main__":
    input = torch.Tensor(1, 64, 128, 128).cuda()
    model = MambaIR(img_size=128, embed_dim=64).cuda()
    model.eval()
    print(model)
    output = model(input)
